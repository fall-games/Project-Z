        ...
        for touch in touches {
            ...
            if location.x > minX && location.x < maxX {
                if location.y > minY && location.y < maxY {
                    ...
                    let width = frame.size.width
                    let rectSize = width / 4
                    let moveLeftButton = SKAction.moveBy(x: -rectSize * 2, y: 0, duration: 0.25)
                    let moveLeftScene = SKAction.moveBy(x: -width, y: 0, duration: 0.25)
                    nodeBs.run(moveLeftScene)
                    nodeB.run(moveLeftButton, completion: {self.changeScene()})
//                } else {
//                    self.touchesMoved(touches, with: event)
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in touches {
            ...
            let nodeBs: SKShapeNode = self.childNode(withName: "BATTLESCENE") as! SKShapeNode
            let nodeB: SKShapeNode = self.childNode(withName: "BATTLEBUTTON") as! SKShapeNode
            if nodeBs.position.x <= 0 && nodeBs.position.x >= -width {
                nodeBs.position.x = nodeBs.position.x + offset
                nodeB.position.x = nodeB.position.x + (offset/ratio)
            }
        }
    }
